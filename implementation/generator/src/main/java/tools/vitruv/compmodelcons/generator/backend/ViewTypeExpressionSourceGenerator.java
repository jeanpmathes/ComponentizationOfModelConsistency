...
public class ViewTypeExpressionSourceGenerator implements ExpressionResolver {
  ...
  public ViewTypeExpressionSourceGenerator(
    ...
    this.expressionHoldingType =
        (JvmGenericType) jvmModelAssociations.getPrimaryJvmElement(viewTypeDefinition);
    this.fileName = this.expressionHoldingType
        .getQualifiedName()
        .replace('.', '/') + ".java";
  }
  ...
  public String generate() {
    return jvmModelGenerator
        .generateType(expressionHoldingType, generatorConfigProvider.get(expressionHoldingType))
        .toString();
  }
  ...
  public String getMethodName(XExpression expression) {
    return logicalContainerProvider
        .getLogicalContainer(expression)
        .getSimpleName();
  }
