...
public class Source implements OriginOperation {
  ...
  public static void attachedCreatedOriginObject(EObject created, EClass sourceClass,
                                                 boolean isRoot, EReference container,
                                                 PutContext context) {
    if (isRoot) {
      ...
      if (candidates.size() == 1) {
        if (container.isMany()) {
          DynamicModels
              .getList(candidates.getFirst(), container)
              .add(created);
        } else {
          candidates
              .getFirst()
              .eSet(container, created);
        }
      }
    }
  ...
  public static void detachDeletedOriginObject(EObject deleted, EClass sourceClass, boolean isRoot,
                                               EReference container, PutContext context) {
    if (isRoot) {
  ...
  public OriginBinding doPut(EChange<EObject> viewChange, OriginBinding target,
    ...
    if (viewChange instanceof CreateEObject<EObject> createEObject) {
      ...
      context
          .getCorrespondences()
          .addCorrespondence(List.of(created), createEObject.getAffectedElement());
    ...
    if (viewChange instanceof DeleteEObject<EObject> deleteEObject) {
      EObject deleted = target
          .originObjects()
          .getFirst();
      context
          .getCorrespondences()
          .removeCorrespondence(List.of(deleted), deleteEObject.getAffectedElement());
