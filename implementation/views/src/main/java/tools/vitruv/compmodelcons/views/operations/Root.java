...
public class Root {
  ...
  public Root(EClass rootClass, Optional<Project> root, List<Target> targets) {
    ...
    for (int index = 0; index < targets.size(); index++) {
      targetIndices.put(targets
                            .get(index)
                            .reference()
                            .getEReferenceType(), index);
      targetContainmentReferences.add(targets
                                          .get(index)
                                          .reference());
    }
  }
  ...
  public ViewBinding doGet(GetContext context) {
    List<RootObjectBindingImpl> roots =
        root
            ...
            .map(rootBinding -> {
              ...
              return new RootObjectBindingImpl(rootBinding, targets
                  .stream()
                  .map(entry -> {
                    Map<EObject, ObjectBinding> result = new HashMap<>();
                    for (ObjectBinding targetBinding : entry
                        .operation()
                        .beginGetByCreatingViewObjects(context)) {
                      result.put(targetBinding.viewObject(), targetBinding);
                      DynamicModels
                          .getList(rootBinding.viewObject(), entry.reference())
    ...
    for (RootObjectBindingImpl rootBinding : roots) {
      ...
      for (int targetIndex = 0; targetIndex < targets.size(); targetIndex++) {
        for (var targetBinding : rootBinding.targetBindings
            .get(targetIndex)
            .values()) {
          targets
              .get(targetIndex)
  ...
  public ViewBinding doPut(EChange<EObject> viewChange, ViewBinding viewBinding,
                           PutContext context) {
    ...
    if (responsibleRootIndex == -1 && !affectedViewObject
        .eClass()
        .equals(rootClass)) {
  ...
  private RootObjectBindingImpl doPut(EChange<EObject> change, ObjectBinding target,
                                      PutContext context) {
    ...
    if (affectedViewObject
      ...
      if (target
        ...
        return new RootObjectBindingImpl(rootBinding,
                                         Stream
                                             .generate(
                                                 () -> (Map<EObject, ObjectBinding>) new HashMap<EObject, ObjectBinding>())
                                             .limit(targets.size())
                                             .toList()
        );
      } else {
      ...
      ObjectBinding peeledTarget =
          Optional
              .ofNullable(rootTarget.targetBindings
                              .get(classIndex)
      ...
      ObjectBinding targetBinding =
          targets
              .get(classIndex)
              .operation()
              .doPut(change, peeledTarget, context);
      if (targetBinding
          .originObjects()
          .isEmpty()) {
        rootTarget.targetBindings
            .get(classIndex)
            .remove(affectedViewObject);
      } else {
        rootTarget.targetBindings
            .get(classIndex)
            .put(affectedViewObject, targetBinding);
      }
  ...
  public ViewBinding doUpdatingGet(ViewBinding previous, EChange<EObject> originChange,
                                   GetContext context) {
    return null;
  }
