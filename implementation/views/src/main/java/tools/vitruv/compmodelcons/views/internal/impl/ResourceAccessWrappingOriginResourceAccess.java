...
public class ResourceAccessWrappingOriginResourceAccess extends AbstractOriginResourceAccess
    implements OriginResourceAccess {
  ...
  protected Collection<EObject> getRoots() {
    return Streams
        .concat(resourceAccess
                    .getModelResources()
                    .stream(), Arrays.stream(additionalResources))
  ...
  protected boolean canDeriveNameFromPackage(EPackage ePackage) {
    return Arrays
        .stream(additionalResources)
        .noneMatch(resource -> resource
            .getContents()
            .stream()
            .anyMatch(root -> root
                .eClass()
  ...
  public void createResourceWithRoot(URI uriHint, EObject root) {
    resourceAccess.persistAsRoot(root, determineOriginUri(root
                                                              .eClass()
                                                              .getEPackage(), uriHint));
