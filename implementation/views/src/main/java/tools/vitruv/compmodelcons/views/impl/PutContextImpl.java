...
public class PutContextImpl extends GetContextImpl implements PutContext {
  ...
  public PutContextImpl(OriginResourceAccess originResourceAccess,
                        ViewResourceAccess viewResourceAccess,
                        EditableViewCorrespondences correspondences, ViewObserver viewObserver) {
  ...
  public void addRootToDefaultOriginModel(EPackage originPackage, EObject originObject) {
    getOriginResourceAccess()
        .getDefaultResource(originPackage)
        .ifPresentOrElse(resource -> resource
            .getContents()
  ...
  @Override
  public void moveRootToOtherOriginModel(EPackage originPackage, EObject originObject,
                                         URI uriHint) {
  ...
  public void moveRootToDefaultOriginModel(EPackage originPackage, EObject originObject) {
    if (originObject.eResource() == null) {
      throw new IllegalArgumentException(
          "Cannot move an origin object which is not already in a resource");
    }
  ...
  public void validateAttachmentState() {
    ...
    if (!unattachedCreatedOriginObjects.isEmpty()) {
      throw new IllegalStateException(
          "Failed to attach all created objects in the origin models, possibly because of " +
              "ambiguous containment");
    }
    if (!undetachedDeletedOriginObjects.isEmpty()) {
      throw new IllegalStateException(
          "Failed to detach all deleted objects in the origin models, possibly because of " +
              "ambiguous containment");
    }
  }
}
